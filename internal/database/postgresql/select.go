...
func GetPagination(c *gin.Context) Pagination {
    ...
    // Check if the "page" query parameter is present and valid
    if c.Query("page") != "" {
    ...
    // Check if the "limit" query parameter is present and valid
    if c.Query("limit") != "" {
    ...
    // Return Pagination struct based on the presence of page and limit values
...
// SelectAllFromDb retrieves all records from the database that match the given query and stores them in the receiver.
// Parameters:
// - db: The database connection.
// - order: The order in which to sort the results (asc or desc). Defaults to "desc" if empty.
// - receiver: The variable to store the retrieved records.
// - query: The query conditions to filter the records.
// - args: Additional arguments for the query conditions.
